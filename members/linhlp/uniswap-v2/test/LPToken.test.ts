...
describe("LPToken", async function () {
  ...
  it("should remove liquidity", async function () {
    const { networkHelpers } = await network.connect();
    const { lpTokenAsOwner, owner } = await networkHelpers.loadFixture(
      deployFixture
    );
  ...
  it("should swap exact out (buy)", async function () {
    const { networkHelpers } = await network.connect();
    const { lpTokenAsOwner, token1AsOwner } = await networkHelpers.loadFixture(
      deployFixture
    );
    ...
    // Get token addresses
    const token0 = await lpTokenAsOwner.read.token0();
    const token1 = await lpTokenAsOwner.read.token1();
    ...
    const maxAmountIn = parseEther("1000");

    const txPromise = lpTokenAsOwner.write.swapExactOut([
      amountOut,
      token1, // tokenIn
      maxAmountIn,
    ]);
  ...
  it("should swap exact out (sell)", async function () {
    const { networkHelpers } = await network.connect();
    const { lpTokenAsOwner, token0AsOwner } = await networkHelpers.loadFixture(
      deployFixture
    );
    ...
    const maxAmountIn = parseEther("10");
