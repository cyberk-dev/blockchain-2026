...
describe("TokenFactory", async function () {
  ...
  async function deployFixture() {
    ...
    const factoryAsCreator = await viem.getContractAt(
      "TokenFactory",
      factory.address,
      {
  ...
  it("createToken requires creation fee", async function () {
    ...
    const { factoryAsCreator } = await networkHelpers.loadFixture(
      deployFixture
    );
  ...
  it("createToken succeeds with correct fee", async function () {
    const { networkHelpers } = await network.connect();
    const { factoryAsCreator, factory } = await networkHelpers.loadFixture(
      deployFixture
    );
  ...
  it("creation fee is sent to feeRecipient", async function () {
    const { networkHelpers } = await network.connect();
    const { factoryAsCreator, feeRecipient } = await networkHelpers.loadFixture(
      deployFixture
    );
