...
describe("Token", async function () {
  ...
  async function deployFixture() {
    const [owner, buyer, feeRecipient] = await viem.getWalletClients();
    const publicClient = await viem.getPublicClient();

    const mockUSDT = await viem.deployContract("MockToken", []);
    ...
    return { owner, buyer, feeRecipient, mockUSDT, token, tokenAsBuyer };
  }
  ...
  it("getCost returns correct price for first token", async function () {
    ...
    const cost = await token.read.getCost([0n, ONE_TOKEN]);
  ...
  it("getCost returns correct price for next 10 tokens", async function () {
    ...
    const cost = await token.read.getCost([ONE_TOKEN, 10n * ONE_TOKEN]);
  ...
  it("buyToken succeeds", async function () {
    const { networkHelpers } = await network.connect();
    const { token, tokenAsBuyer, buyer } = await networkHelpers.loadFixture(
      deployFixture
    );
  ...
  it("ERC20 balances change correctly", async function () {
    const { networkHelpers } = await network.connect();
    const { token, tokenAsBuyer, mockUSDT, buyer, feeRecipient } =
      await networkHelpers.loadFixture(deployFixture);

    const feeAmount = (EXPECTED_FIRST_TOKEN_COST * BUY_FEE_PERCENT) / 100n;
    const netAmount = EXPECTED_FIRST_TOKEN_COST - feeAmount;
    ...
    await viem.assertions.erc20BalancesHaveChanged(
      ...
      [
        ...
        { address: token.address, amount: netAmount },
        { address: feeRecipient.account.address, amount: feeAmount },
      ]
    );
  });
  ...
  it("buy fee splits correctly between contract and feeRecipient", async function () {
    const { networkHelpers } = await network.connect();
    const { token, tokenAsBuyer, mockUSDT, buyer, feeRecipient } =
      await networkHelpers.loadFixture(deployFixture);
  ...
  it("emits TokensPurchased event with correct parameters", async function () {
    const { networkHelpers } = await network.connect();
    const { token, tokenAsBuyer, buyer } = await networkHelpers.loadFixture(
      deployFixture
    );
    ...
    await viem.assertions.emitWithArgs(txPromise, token, "TokensPurchased", [
