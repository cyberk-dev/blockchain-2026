...
export default async (): Promise<Partial<NetworkHooks>> => {
  const handlers: Partial<NetworkHooks> = {
    async newConnection<ChainTypeT extends ChainType | string>(
      ...
      (connection.viem.assertions as any).erc20BalancesHaveChanged = async (
        resolvedTxHash: Promise<string> | string,
        token: any,
        changes: Array<{ address: string; amount: bigint }>,
        diff: bigint = 0n
      ) => {
        ...
        const hash = (await resolvedTxHash) as `0x${string}`;
        ...
        const afterBalances = await Promise.all(
          changes.map(({ address }) =>
            publicClient.readContract({
              abi,
              address: ((token as any)?.address || token) as `0x${string}`,
              functionName: "balanceOf",
              args: [address as `0x${string}`],
            })
          )
        ...
        changes.forEach(({ address, amount }, index) => {
          ...
          assert.ok(
            delta <= diff,
            `For address "${address}", expected balance to change by ${amount} (from ${balanceBefore} t${
